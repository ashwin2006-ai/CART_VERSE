...
export const ShopProvider = ({ children }) => {
  ...
  const adminAddProduct = useCallback((productData) => {
    return apiClient.createProduct(productData).then(res => {
      if (res.success && res.data) {
        setProducts(prev => [...prev, res.data]);
        addToast({ type: 'success', title: 'Product Added', message: 'Product created successfully.' });
        return { success: true };
      } else {
        addToast({ type: 'error', title: 'Failed', message: res.message || 'Failed to create product.' });
  ...
  const adminUpdateProduct = useCallback((productId, productData) => {
    return apiClient.updateProduct(productId, productData).then(res => {
      if (res.success && res.data) {
        ...
        addToast({ type: 'error', title: 'Failed', message: res.message || 'Failed to update product.' });
        return { success: false };
      }
    }).catch(err => {
      addToast({ type: 'error', title: 'Error', message: err.message });
  ...
  const adminDeleteProduct = useCallback((productId) => {
    return apiClient.deleteProduct(productId).then(res => {
      if (res.success) {
        setProducts(prev => prev.filter(p => p.id !== productId));
        addToast({ type: 'success', title: 'Deleted', message: 'Product deleted successfully.' });
        return { success: true };
      } else {
        addToast({ type: 'error', title: 'Failed', message: res.message || 'Failed to delete product.' });
        return { success: false };
      }
    }).catch(err => {
      addToast({ type: 'error', title: 'Error', message: err.message });
  ...
  const adminUpdateInventory = useCallback((productId, stock) => {
    return apiClient.updateProductStock(productId, stock).then(res => {
      if (res.success && res.data) {
        setProducts(prev => prev.map(p => p.id === productId ? res.data : p));
        addToast({ type: 'success', title: 'Stock Updated', message: 'Product stock updated.' });
        return { success: true };
      } else {
        addToast({ type: 'error', title: 'Failed', message: res.message || 'Failed to update stock.' });
        return { success: false };
      }
    }).catch(err => {
      addToast({ type: 'error', title: 'Error', message: err.message });
  ...
  const adminAddCategory = useCallback((categoryData) => {
    return apiClient.createCategory(categoryData).then(res => {
      if (res.success && res.data) {
        setCategories(prev => [...prev, res.data]);
        addToast({ type: 'success', title: 'Category Added', message: 'Category created successfully.' });
        return { success: true };
      } else {
        addToast({ type: 'error', title: 'Failed', message: res.message || 'Failed to create category.' });
        return { success: false };
      }
    }).catch(err => {
      addToast({ type: 'error', title: 'Error', message: err.message });
  ...
  const adminDeleteCategory = useCallback((categoryId) => {
    return apiClient.deleteCategory(categoryId).then(res => {
      if (res.success) {
        setCategories(prev => prev.filter(c => c.id !== categoryId));
        addToast({ type: 'success', title: 'Deleted', message: 'Category deleted successfully.' });
        return { success: true };
      } else {
        addToast({ type: 'error', title: 'Failed', message: res.message || 'Failed to delete category.' });
        return { success: false };
      }
    }).catch(err => {
      addToast({ type: 'error', title: 'Error', message: err.message });
  ...
  const adminAddCoupon = useCallback((couponData) => {
    return apiClient.createCoupon(couponData).then(res => {
      if (res.success && res.data) {
        setCoupons(prev => [...prev, res.data]);
        addToast({ type: 'success', title: 'Coupon Added', message: 'Coupon created successfully.' });
        return { success: true };
      } else {
        addToast({ type: 'error', title: 'Failed', message: res.message || 'Failed to create coupon.' });
        return { success: false };
      }
    }).catch(err => {
      addToast({ type: 'error', title: 'Error', message: err.message });
  ...
  const adminDeleteCoupon = useCallback((couponId) => {
    return apiClient.deleteCoupon(couponId).then(res => {
      if (res.success) {
        setCoupons(prev => prev.filter(c => c.id !== couponId));
        addToast({ type: 'success', title: 'Deleted', message: 'Coupon deleted successfully.' });
        return { success: true };
      } else {
        addToast({ type: 'error', title: 'Failed', message: res.message || 'Failed to delete coupon.' });
        return { success: false };
      }
    }).catch(err => {
      addToast({ type: 'error', title: 'Error', message: err.message });
  ...
  const adminToggleCoupon = useCallback((couponId, active) => {
    return adminUpdateCoupon(couponId, { active });
  }, []);

  // Placeholder for update coupon
  const adminUpdateCoupon = useCallback((couponId, couponData) => {
    return apiClient.updateCoupon(couponId, couponData).then(res => {
      if (res.success && res.data) {
        setCoupons(prev => prev.map(c => c.id === couponId ? res.data : c));
        addToast({ type: 'success', title: 'Updated', message: 'Coupon updated successfully.' });
        return { success: true };
      } else {
        addToast({ type: 'error', title: 'Failed', message: res.message || 'Failed to update coupon.' });
        return { success: false };
      }
    }).catch(err => {
      addToast({ type: 'error', title: 'Error', message: err.message });
  ...
  const value = {
    ...
    adminToggleCoupon,
    adminUpdateCoupon,
    changeAdminPassword: useCallback((currentPassword, newPassword) => {
      return apiClient.changeAdminPassword(currentPassword, newPassword).then(res => {
        if (res.success) {
          addToast({ type: 'success', title: 'Password Changed', message: 'Your password has been updated.' });
          return { success: true };
        } else {
          addToast({ type: 'error', title: 'Failed', message: res.message || 'Failed to change password.' });
          return { success: false };
        }
      }).catch(err => {
        addToast({ type: 'error', title: 'Error', message: err.message });
    ...
    updateAdminProfile: useCallback((profileData) => {
      return apiClient.updateAdminProfile(profileData).then(res => {
        if (res.success && res.data) {
          setAdminAuth(prev => ({
            ...prev,
            adminUser: { ...prev.adminUser, ...res.data }
          }));
          addToast({ type: 'success', title: 'Profile Updated', message: 'Your profile has been saved.' });
          return { success: true };
        } else {
          addToast({ type: 'error', title: 'Failed', message: res.message || 'Failed to update profile.' });
          return { success: false };
        }
      }).catch(err => {
        addToast({ type: 'error', title: 'Error', message: err.message });
